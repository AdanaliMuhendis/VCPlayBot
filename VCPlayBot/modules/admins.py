...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Yayın Durdu!")
    else:
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Sırada Parça Yok!")
    else:
        try:
...
async def skip(_, message: Message):
    ...
    chat_id = get_chat_id(message.chat)
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Oynatılacak Başka Parça Yok!")
    else:
    ...
    if not qeue:
        return
    await message.reply_text(f"- Parça Atlandı **{skip[0]}**\n- Şimdi Oynatılan **{qeue[0][0]}**")
