...
async def update_admin(client, message: Message):
    ...
    await message.reply_text("❇️ Admin Listesi Yenilendi!")


@Client.on_message(command("pause", "dur") & other_filters)
@errors
...
async def pause(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Hiç Bir Şey Oynatılmıyor!")
    else:
        callsmusic.pause(chat_id)
        await message.reply_text("▶️ Yayın Durduruldu!")
...
async def resume(_, message: Message):
    ...
    if (chat_id not in callsmusic.active_chats) or (
        ...
        await message.reply_text("❗ Durdurulacak Bir Şey Yok!")
    else:
        callsmusic.resume(chat_id)
        await message.reply_text("⏸ Yayın Dewamkeee!")
...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Yayın Aktif Değil!")
    else:
        try:
        ...
        await message.reply_text("❌ Yayın Sonlandırıldı!")


@Client.on_message(command("skip", "atla", "lehaaa") & other_filters)
@errors
@authorized_users_only
async def skip(_, message: Message):
    ...
    chat_id = get_chat_id(message.chat)
    if chat_id not in callsmusic.active_chats:
        await message.reply_text("❗ Değiştirilecek Başka Parça Yok!")
    else:
    ...
    if not qeue:
        return
    await message.reply_text(f"- Parça Değiştiriliyor **{skip[0]}**\n- Şuan Oynatılan **{qeue[0][0]}**")
